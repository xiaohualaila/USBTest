    ...
    private Camera.PictureCallback jpeg = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            try {
                ...
                boolean isNetAble = MyUtil.isNetworkAvailable(CameraActivity.this);
                if(isNetAble){
                    uploadPhoto();
                }else {
                ...
                while ((ComData = QueueList.poll()) != null) {
                    runOnUiThread(new Runnable() {
                        public void run() {
                            if(!isReading){
                                if (kk) {
                                    str = new String(ComData.bRec).trim();
                                    Log.i("xxx", "二维码》》》》  " + str);
                                    kk = false;
                                } else {
                                    kk = true;
                                    type = 2;
                                    ticketNum = str.trim() + new String(ComData.bRec).trim();
                                    Log.i("xxx", "二维码》》》》  " + str + new String(ComData.bRec).trim());
                                    isReading =true;
                                    takePhoto();
                                }
                            }
